    ...
    return (
        <div className="flex flex-col items-center pt-12 pb-8 text-center">
            <div className="relative h-32 w-32 overflow-hidden rounded-full ring-4 ring-gray-50 md:h-40 md:w-40">
                <img src={image.startsWith('http') ? image : `/images/${image.split('/').pop()}`} alt={name} className="h-full w-full object-cover" />
            </div>
            ...
            <div className="mt-4 max-w-sm space-y-2 text-base text-gray-500 md:text-lg">
                {bio.map((paragraph, i) => (
                    <p key={i}>{paragraph}</p>
                ))}
            </div>

# 프로필 사진 & 유튜브 커버 — Soul 2.0 프롬프트 킷

> 사용법: Higgsfield → Soul 2.0 → 캐릭터 **urban chic muse** 선택 → 아래 프롬프트 붙여넣기.
> 톤 기준: `creative-direction.md` 보정판 — **Vogue가 아니라 작업실의 쿨함**. 열심히 일하는 사람의 세련됨. 럭셔리 소품·과한 글램 금지.

---

## 공통 스타일 DNA (모든 프롬프트에 이미 포함됨)

- **무드**: sunlit creative studio — 실제로 일하는 사람의 공간 (연출된 호텔 라운지 X)
- **팔레트**: warm neutrals (ivory, oatmeal, camel) + 포인트 1색 (cobalt blue 추천 — 니트나 소품 하나에만)
- **옷**: relaxed tailoring — 화이트 티 위에 오버사이즈 블레이저 또는 니트, **소매는 걷어올림**(허슬 시그널), 주얼리 미니멀
- **표정**: 카메라와 눈높이 맞춘 따뜻한 자신감 — 살짝 웃거나 웃음이 터지기 직전
- **빛**: 창문 자연광, 부드러운 그림자, 약한 필름 그레인
- **금지**: 명품 로고, 이브닝 글램 메이크업, 잡티 제로의 과보정 피부, 스톡포토식 포즈(팔짱+45도), 순백 스튜디오 배경의 차가움

---

## A. 프로필 사진 (프사) — 비율 1:1 (없으면 4:5로 뽑아 정방 크롭)
...
profile, warm and approachable rather than formal.
```

---

## B. 유튜브 커버 (채널 아트) — 비율 16:9, 최대 해상도로 생성

> 유튜브 규격: **2560×1440**, 모든 기기에서 보이는 안전영역은 **중앙 1546×423**뿐.
> → 인물과 핵심 요소는 **가로 중앙 60% 안**에, 상하로는 **정중앙 밴드**에 오도록 뽑는다.
> → 생성 후 최종 조립(리사이즈·여백 확장·텍스트 얹기)은 아래 '후반 작업'대로.

**B-1 · 메인 추천 — 아침의 작업실 (와이드)**
```
Wide 16:9 cinematic shot of the character working at a warm wooden desk in
...
workspace that looks genuinely used.
```
...
who shows her work, warm neutrals, subtle grain.
```
...
## C. 후반 작업 (생성 결과를 받은 뒤 — Claude에게 이미지만 주면 처리)

1. **프사**: 원형 크롭 시뮬레이션 확인 → 얼굴 비중 부족하면 크롭 보정 → 800×800 PNG 내보내기 (IG/유튜브/LinkedIn 공용)
2. **커버**: 2560×1440로 리사이즈/여백 확장 → 안전영역(중앙 1546×423) 가이드에 인물·시선 배치 확인 → 채널명 확정 전이므로 텍스트 없이, 또는 태그라인만 좌측 여백에 작게: *"I show my work — all of it."*
3. 모바일/데스크톱/TV 크롭 3종 미리보기 생성해서 확인
